...
function newGame() {
    // Stop any ongoing game or timer
  ...
  if (!isFirstLoad) {
    // Hide the focus prompt immediately when starting a new game
    ...
    // Position the cursor before the first letter
    const firstLetter = document.querySelector('.letter.current');
    const cursor = document.getElementById('cursor');
    const rect = firstLetter.getBoundingClientRect();
    cursor.style.top = rect.top + 2 + 'px'; // Adjust the `+2` to fine-tune vertical alignment
    cursor.style.left = rect.left + 'px';
  
    // Clear and reattach keyup listener
    document.getElementById('game').removeEventListener('keydown', keyupHandler);
    document.getElementById('game').addEventListener('keydown', keyupHandler);
  }
...
document.getElementById('game').addEventListener('keydown', ev => {
  ...
  if (isBackspace) {
    ...
    if (extraLetters.length > 0) {
      ...
      // Move to previous word's last letter
      removeClass(currentWord, 'current');
      ...
      // Move back one letter, reset its state
      removeClass(currentLetter, 'current');
      ...
      // Handle backspace at the start of a word
